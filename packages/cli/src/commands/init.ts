...
The developer just ran \`trellis-sq init\` on this project for the first time.
...
function getBootstrapTaskJson(
  ...
  return emptyTaskJson({
    ...
    createdAt: today,
    relatedFiles,
    notes: `First-time setup task created by trellis-sq init (${projectType} project)`,
  });
}
...
function getJoinerTaskJson(developer: string, taskName: string): TaskJson {
  ...
  return emptyTaskJson({
    ...
    createdAt: today,
    notes:
      "Generated by trellis-sq init for a new developer joining an existing Trellis project",
  });
}
...
\`${developer}\` just ran \`trellis-sq init\` on a fresh clone, saw "Developer
...
"Welcome! Your \`trellis-sq init\` set me up to onboard you to this project. I
  ...
  if (selectedTemplate) {
    ...
    if (result.success) {
      ...
      const retryCmd = registry
        ? `trellis-sq init --registry ${registry.gigetSource} --template ${selectedTemplate}`
        : `trellis-sq init --template ${selectedTemplate}`;
    ...
    if (result.success) {
      ...
      console.log(
        chalk.gray(
          `   You can retry later: trellis-sq init --registry ${registry.gigetSource}`,
        ),
      );
    }
  }
